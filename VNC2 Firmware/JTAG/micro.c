...
*
*	This file has been ported to the VNC2 by John Sully.  Read back verification
*	has been removed for performance reasons.
*****************************************************************************/
...
extern rom const unsigned char jtag_bitvals[256][16];
/*****************************************************************************
    ...
    while ( lNumBits )
    {
		...
		else
		{
			...
			for(iout = 0; iout < 16; iout++)
				porta_out = jtag_bitvals[*pucTdi][iout] | LED_GREEN_OFF;
			lNumBits -= 8;
		}
...
int xsvfRun( SXsvfInfo* pXsvfInfo )
{
    ...
    if ( (!pXsvfInfo->iErrorCode) && (!pXsvfInfo->ucComplete) )
    {
        ...
        if ( pXsvfInfo->ucCommand < XLASTCMD )
        {
			...
			{
			case	0:    xsvfDoXCOMPLETE( pXsvfInfo ); break;        /*  0 */
			case	1:	xsvfDoXTDOMASK( pXsvfInfo ); break;         /*  1 */
			case	2:	xsvfDoXSIR( pXsvfInfo ); break;             /*  2 */
			case	3:	xsvfDoXSDR( pXsvfInfo ); break;            /*  3 */
			case	4:	xsvfDoXRUNTEST( pXsvfInfo ); break;         /*  4 */
			case	5:	xsvfDoIllegalCmd( pXsvfInfo ); break;       /*  5 */
			case	6:	xsvfDoIllegalCmd( pXsvfInfo ); break;       /*  6 */
			case	7:	xsvfDoXREPEAT( pXsvfInfo ); break;          /*  7 */
			case	8:	xsvfDoXSDRSIZE( pXsvfInfo ); break;         /*  8 */
			case	9:	xsvfDoXSDRTDO( pXsvfInfo ); break;          /*  9 */
				#ifdef  XSVF_SUPPORT_COMPRESSION
			case	10:	xsvfDoXSETSDRMASKS( pXsvfInfo ); break;     /* 10 */
			case	11:	xsvfDoXSDRINC( pXsvfInfo ); break;          /* 11 */
				#else
			case	10:	xsvfDoIllegalCmd,       /* 10 */
			case	11:	xsvfDoIllegalCmd,       /* 11 */
				#endif  /* XSVF_SUPPORT_COMPRESSION */
			case	12:	xsvfDoXSDRBCE( pXsvfInfo ); break;          /* 12 */
			case	13:	xsvfDoXSDRBCE( pXsvfInfo ); break;          /* 13 */
			case	14:	xsvfDoXSDRBCE( pXsvfInfo ); break;          /* 14 */
			case	15:	xsvfDoXSDRTDOBCE( pXsvfInfo ); break;       /* 15 */
			case	16:	xsvfDoXSDRTDOBCE( pXsvfInfo ); break;       /* 16 */
			case	17:	xsvfDoXSDRTDOBCE( pXsvfInfo ); break;       /* 17 */
			case	18:	xsvfDoXSTATE( pXsvfInfo ); break;           /* 18 */
			case	19:	xsvfDoXENDXR( pXsvfInfo ); break;           /* 19 */
			case	20:	xsvfDoXENDXR( pXsvfInfo ); break;           /* 20 */
			case	21:	xsvfDoXSIR2( pXsvfInfo ); break;            /* 21 */
			case	22:	xsvfDoXCOMMENT( pXsvfInfo ); break;         /* 22 */
			case	23:	xsvfDoXWAIT( pXsvfInfo ); break;            /* 23 */
			}
        }
        else
        {
...
int xsvfExecute()
{
	...
	int it;
    SXsvfInfo  *pxsvfInfo;
	pxsvfInfo = malloc(sizeof(SXsvfInfo));
	pbjtag = malloc(JTAG_BUFFER_SIZE);

    xsvfInitialize( pxsvfInfo );
    ...
    xsvfCleanup( pxsvfInfo );
	ret = XSVF_ERRORCODE(pxsvfInfo->iErrorCode);
	free(pxsvfInfo);
	free(pbjtag);
    return( ret );
}
